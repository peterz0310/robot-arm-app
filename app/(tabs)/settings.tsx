...
import {
  ...
  JointId,
  useRobotController,
} from "@/hooks/use-robot-controller";
    ...
    >
      <Section title="Connection">
        ...
        <TextInput
          ...
          placeholder="ws://192.168.1.100:80/arm"
        ...
        <Text style={styles.hint}>
          Format: ws://IP_ADDRESS:PORT/PATH (e.g., ws://192.168.1.100:80/arm)
        </Text>
      ...
      <Section title="Joint limits & home">
        <Text style={styles.hint}>
          Clamp ranges per servo. These values gate every outgoing payload so
          the ESP32 only receives safe angles.
        </Text>
            ...
            <View key={opt.id} style={styles.jointCard}>
              <View style={styles.row}>
                ...
                <Text style={styles.label}>
                  Current {jointAngles[opt.id].toFixed(1)}°
                </Text>
              </View>
              <View style={styles.triple}>
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(txt) =>
                      handleNumberChange(opt.id, "min", txt)
                    }
                    style={styles.input}
                  />
                </View>
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    onChangeText={(txt) =>
                      handleNumberChange(opt.id, "max", txt)
                    }
                    style={styles.input}
                  />
                </View>
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    keyboardType="numeric"
                    onChangeText={(txt) =>
                      handleNumberChange(opt.id, "home", txt)
                    }
                    style={styles.input}
                  />
                </View>
              ...
              <Pressable
                ...
                onPress={() =>
                  setJointConfig(opt.id, { home: jointAngles[opt.id] })
                }
              >
      ...
      <Section title="Gyro control">
        <Text style={styles.hint}>
          Use phone pitch/roll as two virtual sliders. Calibrate while the arm
          is in its home pose.
        </Text>
        <View style={styles.row}>
          <Pressable
            style={[
              styles.button,
              {
                backgroundColor: settings.gyroEnabled
                  ? palette.accent
                  : palette.elevated,
                borderColor: palette.border,
                borderWidth: 1,
          ...
          >
            <Text
              style={[
                styles.buttonText,
                { color: settings.gyroEnabled ? "#041015" : palette.text },
              ]}
            >
            ...
            <TextInput
              ...
              onChangeText={(txt) => {
                ...
                if (Number.isFinite(value))
                  updateSettings({ gyroScale: value });
              }}
              style={styles.input}
            />
          </View>
        ...
        <Text style={styles.hint}>
          ...
          {jointConfigs[settings.gyroPitchJoint ?? "armA"]?.label ?? "Arm A"} ·
          Roll→
